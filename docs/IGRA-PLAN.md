...
Страницата (`resources/views/filament/pages/game-stats.blade.php`) съдържа и секция „Линкове към игрите“: базови линкове за бал/сватба, генератор по локация (Filament форма `target` + `loc`, санитизиран през `GameController::sanitizeLoc()`, копиране с Alpine + `FilamentNotification`), списък на вече сканираните стикери с линковете им, и кратка инструкция „Как да валидираш код от Instagram DM“. `GameController::url($target, $loc)` е общият builder на публичния адрес. Ресурсът със събитията е със label „QR Игра – събития“ (sort 2) в същата група.
