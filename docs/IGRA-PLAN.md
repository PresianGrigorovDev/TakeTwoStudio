...
| `app/Filament/Widgets/GameStatsWidget.php` + `resources/views/filament/widgets/game-stats.blade.php` | create | Статистика по target/loc: сканирания, победи, загуби, ваучери, споделяния, конверсия % |
...
`GameStatsWidget` (по образец `PromoCodeStatsWidget`): `$sort = 11`, `columnSpan 'full'`, `$view = 'filament.widgets.game-stats'`. Една заявка: `SELECT target, loc, SUM(CASE WHEN event='scan' THEN 1 ELSE 0 END) scans, … wins, losses, vouchers, shares FROM game_events GROUP BY target, loc` (CASE WHEN → работи и в SQLite за тестове), сортиране по scans desc, конверсия `wins/scans` и `vouchers/scans` със zero guard; null `loc` → „(без локация)“. Blade: секция „QR Игра – статистика по локация“, таблица на target (Абитуриентски бал / Сватба) с колони Локация | Сканирания | Победи | Загуби | Ваучери | Споделяния | Конверсия, tfoot тотали, empty state „Все още няма събития от играта.“, линк „Провери код от DM“ към ресурса.
...
- **Отворено за потвърждение (не блокира старта):** (а) огледалното копие на ваучера в `localStorage`, за да не се губи при затворен таб – препоръчвам „да“; (б) съкращаване на 2–3 най-дълги плочки, ако QA на 360px покаже нечетимост; (в) дали правната страница `igra-usloviya` да влезе в първата версия.
