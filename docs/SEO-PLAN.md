...
|---|---|---|
| Fix duplicate /public/ URLs | root `.htaccess` (deny + 1-hop 301 + rewrite в `public/`), нов `public/.htaccess`, `URL::forceRootUrl/forceScheme`, пренаписан `NormalizeCanonicalUrl` (по `getBaseUrl()`), canonical = `url()->current()`, тестове за всички варианти. **Деплой 2026-09-05:** (1) `/vendor/*` (self-hosted Bootstrap/AOS/FontAwesome/GLightbox в `public/vendor/`) даваше 403, защото deny правилото за Composer `vendor/` хващаше и тези URL-и → добавен guard `RewriteCond %{DOCUMENT_ROOT}/public%{REQUEST_URI} !-f` пред правилото (реален файл под `public/` никога не се блокира). (2) mod_rewrite не наследява root правилата в `public/`, затова заявки, започващи буквално с `/public/`, се канонизират от ново `THE_REQUEST` правило в `public/.htaccess` (root файлът ги вижда само след вътрешния rewrite); Laravel middleware-ът остава fallback. Двата файла са проверени с локален Apache 2.4 върху огледална структура (статични 200, вътрешни 403, `/public/x` → 1×301 с query, POST без redirect, без loop). Известен ръб: голо `/public` без наклонена черта минава през DirectorySlash на сървъра → 2 hop-а до `/` (безвредно). | A, C.1, B3 |
