function calculateBaptismTotal() {
    let total = 0;
    let packagePrice = 0;
    let extrasPrice = 0;
    ...
    selectedExtras.forEach(extra => {
        ...
        // If it's a radio, it might be the "Scope" (Обхват)
        // We can try to detect if it belongs to a group with such a name
        // but it's simpler to just collect all checked extras.
        
        // In the original, 'bap_scope' was special.
        // If we want to keep sumService/sumScope/sumExtras separation:
        if (extra.name.includes('обхват') || (extra.closest('.mb-5') && extra.closest('.mb-5').previousElementSibling && extra.closest('.mb-5').previousElementSibling.innerText.includes('Локация'))) {
             ...
             // Avoid adding 0-price standard options to the list to keep it clean
...
document.addEventListener('DOMContentLoaded', () => {
    calculateBaptismTotal();
    
    // Safety check: ensure all inputs trigger calculation
    const inputs = document.querySelectorAll('.package-option, .extra-option');
    inputs.forEach(input => {
        input.addEventListener('change', calculateBaptismTotal);
    });
});
