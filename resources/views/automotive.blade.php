...
@section('title', 'Автомобилен Фотограф Варна | Car Photography и Фотосесия на Автомобили | Take Two Studio 1603')
@section('meta_description', 'Професионална автомобилна фотография във Варна. Заснемане на автомобили за продажба, автосалони, тунинг проекти и лични колекции. Динамични и статични кадри с перфектно осветление.')
@section('meta_keywords', 'автомобилен фотограф варна, car photography варна, снимки на коли, фотосесия автомобил, снимки за автосалон, тунинг фотография, автомобилни кадри, Take Two Studio')
@section('og_title', 'Автомобилна Фотография Варна — Перфектни Кадри за Вашия Автомобил | Take Two Studio')
                            ...
                            @foreach($groupedExtras as $groupName => $extras)
                                ...
                                <div class="row g-3 mb-5">
                                    ...
                                    <div class="{{ $extra->input_type === 'checkbox' ? 'col-md-4' : 'col-md-6' }}">
                                        <input class="extra-option" type="{{ $extra->input_type }}" name="extra_{{ $extra->id }}{{ $extra->input_type === 'radio' ? '_' . Str::slug($groupName) : '' }}" id="extra_{{ $extra->id }}" value="{{ (int)$extra->price_eur }}" data-label="{{ $extra->label_bg }}" {{ $extra->input_type === 'radio' && $loop->first ? 'checked' : '' }} onchange="calculateGenericTotal()">
...
@push('schema')
@php
$automotiveServiceSchema = [
    '@context' => 'https://schema.org',
    '@type' => 'Service',
    'name' => 'Автомобилна фотография Варна',
    'provider' => [
        '@type' => 'LocalBusiness',
    ...
    'description' => 'Професионално заснемане на автомобили — екстериор, интериор, детайли и динамични кадри. За автосалони, обяви и тунинг проекти.',
    'url' => 'https://taketwostudio1603.com/automotive',
    'serviceType' => 'Автомобилна фотография',
];
@endphp
<script type="application/ld+json">{!! json_encode($automotiveServiceSchema, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) !!}</script>
@endpush
